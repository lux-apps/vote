...
const AddressZero = ethers_1.ZeroAddress;
/**
...
const deployMastercopy = async (signer, mastercopyContractFactory, args, salt) => {
    const deploymentTx = await mastercopyContractFactory.getDeployTransaction(...args);
...
const computeTargetAddress = async (signer, mastercopyContractFactory, args, salt) => {
    ...
    const initCodeHash = (0, ethers_1.keccak256)(deploymentTx.data);
    const computedAddress = (0, ethers_1.getCreate2Address)(await singletonFactory.getAddress(), salt, initCodeHash);
    const targetAddress = (0, ethers_1.getAddress)((await singletonFactory.deploy.staticCall(deploymentTx.data, salt)));
    // Sanity check
...
const deployMastercopyWithInitData = async (signer, initCode, salt) => {
    ...
    const targetAddress = (0, ethers_1.getAddress)((await singletonFactory.deploy.staticCall(initCode, salt)));
    const initCodeHash = (0, ethers_1.keccak256)(initCode);
    const computedTargetAddress = (0, ethers_1.getCreate2Address)(await singletonFactory.getAddress(), salt, initCodeHash);
